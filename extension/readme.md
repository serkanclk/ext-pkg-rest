...
- **Çoklu Sütun Başlığı Seçimi ve Kopyalama**: Sağ tık menüsündeki "Copy Selected Column Headers" artık çoklu seçimi destekliyor. Sütun başlıklarına **tek tıkla** (başlık alanının herhangi bir yerine) seçim yapılır (mavi vurgu), **Ctrl+tıkla** ile seçime ekle/çıkar. Sağ tık → Copy Selected Column Headers ile seçilen sütunlar virgülle ayrılmış kopyalanır. Hiçbir seçim yoksa yalnızca sağ tıklanan sütun kopyalanır.
- **"Copy All Column Headers" Eklendi**: Sağ tık menüsüne yeni seçenek eklendi — seçim durumundan bağımsız olarak tüm sütun adlarını virgülle ayrılmış şekilde kopyalar.
- **Sütun Başlığı Tıklama Alanı Genişletildi**: Sütun seçimi (tek tık) ve sıralama (çift tık) artık yalnızca başlık metnine değil, yeniden boyutlandırma şeridi hariç tüm başlık hücresine tıklanarak yapılabilir. Aynı iyileştirme Data (Object Viewer) panelineetkiledi.
